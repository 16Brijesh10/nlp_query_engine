...
# from app.services.db_utils import get_engine, ensure_employees_table, insert_employees
...
# GLOBAL_QE = None  # will store QueryEngine instance after DB connect
...
#     """Connect to database, discover schema, initialize QueryEngine."""
#     global GLOBAL_QE

#     try:
#         schema_discoverer = SchemaDiscovery(request.connection_string)
#         discovered_schema = schema_discoverer.analyze_database()

#         GLOBAL_QE = QueryEngine(request.connection_string, discovered_schema)
#         GLOBAL_QE.dp = GLOBAL_DP  # link the document processor

#         # store connection string globally for reuse in ingestion
#         from app.api.routes import schema as schema_route
#         schema_route._LAST_CONNECTION_STRING = request.connection_string
...
# ):
#     """
#     Handles file uploads, processes them (chunking/embedding), 
#     and stores both chunks and structured rows (if any) in the database.
#     """
...
#         # 2️⃣ Process documents: returns (chunk_ids, extracted_rows)
#         chunk_ids, extracted_rows = dp.process_documents(saved_paths)

#         # 3️⃣ Try to store chunks + extracted rows in DB
#         try:
#             from app.services.db_utils import (
#                 get_engine, ensure_documents_table, insert_documents,
#                 ensure_employees_table, insert_employees
#             )

#             DATABASE_URL = os.getenv("DATABASE_URL", None)
#             if DATABASE_URL is None:
#                 try:
#                     from app.api.routes.schema import _LAST_CONNECTION_STRING
#                     DATABASE_URL = _LAST_CONNECTION_STRING
#                 except Exception:
#                     DATABASE_URL = None
...
#             else:
#                 print("[Ingestion] No DB URL found; skipping DB persistence.")

#         except Exception as e:
#             print(f"[Ingestion] DB persistence failed or skipped: {e}")
...
#             "inserted_structured_rows": len(extracted_rows)
#         }
...
#     """Mock endpoint for job progress."""
